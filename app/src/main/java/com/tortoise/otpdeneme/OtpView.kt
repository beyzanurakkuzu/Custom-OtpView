...
import com.tortoise.otpdeneme.helpers.fetchColor
...
class OtpView : AppCompatEditText {
    ...
    private var dashColor = Color.BLACK
    ...
    private fun updateColorForLines(i: Int, next: Int, x1: Float, y1: Float, x2: Float, y2: Float) {
        ...
        if (isErrorShown) {
            ...
            if (next < 0 || i == 0) {
                ...
                mLinesPaint!!.shader = null
                mLinesPaint!!.color = context.fetchColor(R.color.black)
            }
    }
    ...
    private fun drawLines(canvas: Canvas) {
        ...
        mCharSize = if (spaceLength <= 0) {
            availableWidth / (lineCountOtp.toFloat() * 2 - 1)
        } else {
            (availableWidth.minus(spaceLength * lineCountOtp.minus(1))) / lineCountOtp.toFloat()
        }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val width: Int = if (widthMode == MeasureSpec.EXACTLY) {
            ...
            val boxesWidth: Int = if(spaceLength > 0)
                ((lineCountOtp - 1) * spaceLength + lineCountOtp * 80).toInt()
            else
                ((2*lineCountOtp - 1) * 80)
